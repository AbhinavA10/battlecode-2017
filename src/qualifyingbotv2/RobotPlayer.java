        ...
        while (true) {
            try {
                ...
                if (movingBackToStart) {
                    ...
                    if (distanceToStart <= 1 && !rc.hasMoved() && rc.canMove(toStartLoc, distanceToStart)) {
                        rc.move(toStartLoc, distanceToStart);
                        currentLoc = rc.getLocation();
                    } else if (!rc.hasMoved() && rc.canMove(toStartLoc)) {
                        rc.move(toStartLoc);
                        currentLoc = rc.getLocation();
                    }
                    ...
                    if (foundStartLoc) {
                        ...
                        if (prevNumScout < SCOUT_MAX && rc.canBuildRobot(RobotType.SCOUT, dirList[6])) {
                ...
                if (foundStartLoc) {
                    ...
                    minTreeHealth = GameConstants.BULLET_TREE_MAX_HEALTH;
                    for (int i = 0; i < nearbyTrees.length; i++) {
                ...
                for (TreeInfo t : nearbyTrees) {
                    if (t.getTeam() != homeTeam) {
                        if (!rc.hasMoved()) {
                            ...
                            if (!rc.hasMoved() && rc.canMove(t.location)) rc.move(t.location);
        ...
        switch (treeArrayLoc) {
            ...
            case 1:
                if (!hasMoved && currentLoc == startLoc) {
                    if (!rc.hasMoved() && rc.canMove(Direction.getSouth())) {
                    ...
                    if (!rc.hasMoved() && rc.canMove(Direction.getSouth(), (float) 0.001)) {
            ...
            case 3:
                if (!hasMoved && currentLoc == startLoc) {
                    if (!rc.hasMoved() && rc.canMove(Direction.getEast())) {
                    ...
                    if (!rc.hasMoved() && rc.canMove(Direction.getEast(), (float) 0.001)) {
            ...
            case 5:
                if (!hasMoved && currentLoc == startLoc) {
                    if (!rc.hasMoved() && rc.canMove(Direction.getNorth())) {
                    ...
                    if (!rc.hasMoved() && rc.canMove(Direction.getNorth(), (float) 0.001)) {
