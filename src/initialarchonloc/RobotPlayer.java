...
public strictfp class RobotPlayer {
   ...
   Used moveDiagonal(), so at least they wouldn't get stuck in a clump
   ...
   **/
   ...
   one defensive lj per gardener, then become defensive
        ...
        * */
        boolean bDefendMode = true;
        int nXPos = 0; // xPos of array
        while (true) {
            try {
                System.out.println(bDefendMode);
                myLocation = rc.getLocation();
                ...
                if (bDefendMode) {
                    ...
                    boolean bGardenerCloseBy = false;
                    RobotInfo[] teammates = rc.senseNearbyRobots(-1, homeTeam);
                    for (RobotInfo b : teammates) {
                        if (b.getType() == RobotType.GARDENER && !rc.hasMoved()) {
                            tryMove(myLocation.directionTo(b.location));
                            bGardenerCloseBy = true;
                            System.out.println("To my gardener");
                        }
                        if (b.getType() == RobotType.LUMBERJACK && bGardenerCloseBy) {
                            bDefendMode = false; // if there is already a lj near the gardener
                            break;
                        }
                    ...
                    if (!rc.hasMoved()) tryMove(dirList[moveOnDiagonal()]);
